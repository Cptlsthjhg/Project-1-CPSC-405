...
enum cmds        { FORK=0, SETPID,   SHOWPID,   WAIT,   EXIT,   SLEEP,   WAKEUP,   PS,   SCHEDULE,  TIMER,    HELP,   QUIT ,  TMOD};
char *cmdstr[] = {"fork", "Setpid", "currpid",  "wait", "exit", "sleep", "wakeup", "ps",   "schedule", "timer", "help", "quit", "tmod"};
...
int executeCmd(char** params, int nparams)
{
    ...
    switch (cmd_index) {
        ...
        case TMOD:
            if(nparams > 2){
                pid = atoi(params[1]);
                int ticketValue = atoi(params[2]);
                int oldTicketValue = Tmod(pid,ticketValue);
                if(oldTicketValue==-1){
                    //tell user that the Tmod failed to find process
                    printf("Failed to find process based off of PID\n");
                }
                else{
                    //tell user that Tmod worked, and what happened.
                    printf("Process %d had %d tickets, now has %d tickets.\n",pid,oldTicketValue,ticketValue);
                }
            }
            else{
                printf("Invalid TMOD usage (parameter 1 is PID, and parameter 2 is ticket value.\n)");
                break;
            }
        case SETPID:
            if (nparams == 1)
